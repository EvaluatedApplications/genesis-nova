...
// learned, not a coded name routine. [SlowFact] because the role parser is LEARNED (no hardcoded copula/possessive
// fallback), so the test must warm it the way the gym warms production - see GrammarWarmup.
public sealed class FactRecallExperiment
{
    ...
    public void Field_LearnsThenRecalls_AnAssertedFact()
    {
        ...
        GrammarWarmup.WarmRoleHead(tok, model, mind); // train the role parser (production warms it via the gym; no hardcoded fallback)
    ...
    public async Task ReplRuntimePath_LearnsThenRecalls()
    {
        ...
        try
        {
            ...
            GrammarWarmup.WarmRoleHead(rt); // warm the role parser as the gym does in production
    ...
    public async Task ReplLearnedFact_SurvivesSaveAndReload()
    {
        ...
        try
        {
            ...
            GrammarWarmup.WarmRoleHead(rt1); // warm the role parser as the gym does in production
