...
public sealed class LivingScaleExperiment
{
    ...
    [SlowFact] // bare subjects ("qabc is fruit") via the LEARNED role parser — needs a GRU-trained warm-up
    public void Does_Recall_Hold_AtScale()
    {
        ...
        GrammarWarmup.WarmRoleHeadWithGym(tok, model, space, config); // train the GRU so the role head tags bare subjects
    ...
    [SlowFact] // bare subjects ("sab is red") — The two living integrations TOGETHER, AT SCALE: over a long run the world keeps CHANGING (belief revision)
